...
- [x] T017+ [US5] Integração em `commands::buscar` (feito): `buscar_por_siape` classifica cada documento válido antes de agrupar; modo default `keyword` (instantâneo, sem API); modo `llm` só ativa com `input.modo == "llm"` **e** chave configurada, senão degrada para `keyword` (R11). `montar_resultado` agora agrupa por `doc.categoria`, na ordem de `config/categoria.json`, omitindo grupos vazios
...
- [x] T042 [P] [US7] Teste: markdown agrupado por categoria — como testes unitários (`#[cfg(test)] mod tests`, mesmo padrão de `carregar_categorias`/T046), não um `tests/relatorio.rs` à parte: `src-tauri/src/services/relatorio.rs` cobre a tabela `| Categoria | Qtd |`, a seção `## Categoria (N)` por grupo, `### N. Título` + meta (Data/SIAPE/Original/Arquivo) + resumo literal (R1 — nunca inventa; documento sem resumo mostra `_(sem resumo)_`), a ordem preservada de `resultado.categorias` (sem reordenar), e um teste de regressão XSS (título/resumo com `<script>`/`<img onerror>` nunca viram tag executável no HTML final — pipeline `gerar_markdown` → `markdown_para_html`)
- [x] T043 [US7] `src-tauri/src/services/relatorio.rs`: `gerar_markdown` (puro) + `markdown_para_html` (puro, via crate `pulldown-cmark` 0.13, feature só `html`) — **decisão revista**: nada de "markdown→PDF via webview"/Chrome headless (frágil, dependência externa de binário instalado no SO, ausente em CI). O relatório vira um **HTML self-contained** (CSS A4 inline, sem assets externos, mesmo CSS de `src/md_para_pdf.py`) salvo ao lado de um `.md`; `commands::exportar::gerar_relatorio` abre o HTML com o app padrão do sistema (`tauri-plugin-opener`), onde "Imprimir → Salvar como PDF" produz um PDF equivalente sem nenhuma dependência nova pesada. `src-tauri/src/services/empacotador.rs` (novo, não previsto nas tasks originais) isola `montar_zip` (crate `zip` 2.x, `default-features = false` — método `Stored`, sem codecs pesados) do resto do relatório, por coesão/tamanho de arquivo
- [x] T044 [US7] Comandos em `src-tauri/src/commands/exportar.rs` (R3,R7) — **nomes revistos**: `gerar_relatorio` (não `gerar_pdf_resumo`, já que não gera PDF — ver T043) e `baixar_zip`, ambos registrados em `commands/mod.rs`/`lib.rs`. `gerar_relatorio` grava `<siape>_relatorio.{html,md}` (nome determinístico, R3, sobrescreve a cada geração) em `<app_data_dir>/relatorios/` e abre o HTML (`opener:allow-open-path`, escopo ampliado para `$APPDATA/relatorios/**` em `capabilities/default.json`). `baixar_zip` valida o SIAPE (R10), monta `<siape>_documentos.zip` a partir de `<app_data_dir>/documentos/<siape>/` e revela o arquivo no gerenciador de arquivos do SO (`opener:allow-reveal-item-in-dir`, nova permissão); sem nenhum PDF baixado ainda, falha com `FalhaArquivo` e mensagem amigável (decisão registrada em `services::empacotador`) em vez de gerar um ZIP vazio. `contracts/ipc-commands.md` atualizado para refletir `gerar_relatorio` no lugar de `gerar_pdf_resumo`
- [x] T045 [P] [US7] Ações do cabeçalho-resumo em `app/src/views/BuscaView.vue` — **extraídas** para `app/src/components/busca/RelatorioAcoes.vue` (novo componente, não previsto nas tasks originais) em vez de inline na View, para isolar esta US de outra branch que também mexe em `BuscaView.vue` (só uma linha de template + o import mudam lá). Botões "Baixar relatório" e "Baixar ZIP" (ícones/rótulos mantidos de `PDF do resumo`/`Baixar ZIP`, ajustados ao que de fato é gerado), habilitados só quando `resultado` tem ao menos 1 item em alguma categoria (R2 pode zerar `categorias` mesmo com `total > 0`), com estado "processando" independente por botão e erro via `mensagemDeErro` (`role="alert"`); alvo clicável `alvo-minimo` (≥ 40px, Constituição XII)
